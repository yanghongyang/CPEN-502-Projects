...
if __name__ == '__main__':
    ...
    plt.xlim(0, max(X_binary_momentum)+20)

    plt.xlabel("Number of epochs")
    plt.ylabel("Error")
    graph = plt.plot(X_binary_momentum, Y_binary_momentum, X_bipolar_momentum, Y_bipolar_momentum)
    plt.setp(graph[0], color='blue', label='binary representation(momentum)')
    plt.setp(graph[1], color='red', label='bipolar representation(momentum)')
    plt.legend(loc = 1)
    plt.title("(1) - c): backpropagation with momentum of \n XOR problem using binary&bipolar representation")
    plt.figure(figsize=(6, 8))
    plt.show()
